...
provider and the SaveConfig base as their own ini files, and clears the
generated ini mirrors. ``compile_mod`` then runs ``UDK.exe make`` and reads the
verdict from the fresh package file and Launch.log. Both are idempotent.
"""
...
def _add_line_to_section(path: Path, section: str, line: str) -> bool:
...
def deploy(install_dir: Path) -> list[str]:
    ...
    # The generated mirrors rebuild from the Default inis on next launch.
    for name in ("UDKEngine.ini", "UDKGame.ini"):
        generated = config / name
        _backup_once(config, generated)
        if generated.is_file():
            generated.unlink()
    log.append("Cleared the generated ini mirrors (they regenerate on launch).")
    return log
